...
一个人处理多个项目,最大的消耗不是某一行代码,而是被反复打断:

上午查 A 项目的 CI,午饭前被问 B 项目能不能发,下午又要回 C 项目的 PR review。每次都要重新进入现场:
...
不要让所有 agents 都直接找老板。  
让 lead 先理解项目背景、仓库环境、历史风险、测试方式,再去指挥 implementer、tester、reviewer。

AICO 当前已经能把本机 Claude Code、Codex、Cursor、Gemini、Trae 等 CLI 通过 Adapter 接进来,用 Telegram 做远程入口,并提供:
