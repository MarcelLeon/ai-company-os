...
我把痛点按真实体感排序拆成 6 个。前两个最痛,因为它们直接决定我敢不敢把任务交出去。
...
一个查问题,一个写代码,一个做 review,听起来很好。但如果它们都直接来问我"下一步做什么",那不是多 agent,那是我多了几个下属窗口。所有 agent 都找老板,老板就成了瓶颈。
...
### P5:AI 很强,但我离开电脑后链路断了

坐在 Mac 前时,我可以盯终端、复制上下文、手动重试、看 diff。可一旦去吃饭、开会、通勤、躺床上,这些动作都没了。

IM 如果只是通知层,不能下任务、审批、打断、接早报,那它解决不了 boss-absent 问题。真正的问题不是"手机能不能收到消息",而是"手机能不能继续管理项目"。

### P6:风险动作不能默认放飞

写文件、跑 shell、`git push`、删除文件,这些不是普通聊天。离线托管如果绕过审批,就会从"帮我干活"变成"替我冒险"。

我希望 AI 能替我推进,但不希望它在我吃饭时替我做不可逆决定。
...
| 痛点 | AICO 的回答 | 为什么这样做 |
|---|---|---|
| P1 长任务不可接手 | Task / Audit / Snapshot / `/task` / `/audit` | 输出可以长,但接手入口必须短 |
| P2 只想看局面,不是看日志 | `/inbox` / `/morning` / `/view` | 手机先看 Boss Brief,深挖再进 trace |
| P3 多 agent 增加调度成本 | Project / Role / Appointment / Team 领域模型 | 先建组织语义,再谈多 agent |
| P4 项目知识反复重讲 | Memory + Experience 分层注入 prompt | fact 和 lesson 是两类东西 |
| P5 人离开电脑后链路断 | IM-first:Telegram 里下任务、审批、打断、看早报 | IM 不是通知层,而是远程管理层 |
| P6 风险动作不可放飞 | RiskLevel + ApprovalPolicy + adapter capability gate | 离线托管不等于 YOLO |
