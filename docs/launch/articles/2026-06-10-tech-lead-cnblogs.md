...
| 优先级 | 技术痛点 | 如果不解决会怎样 | AICO 的模型 |
|---|---|---|---|
| 1 | 所有 agent 都找老板 | 老板成为调度瓶颈,多 agent 反而制造更多小决策 | Lead responsibility + default role |
| 2 | 权限不可控 | read-only reviewer 可能被派去执行 shell 或危险动作 | RiskLevel + ApprovalPolicy + capability gate |
| 3 | IM 输出不可读 | 手机上看到的是一整屏日志,无法快速判断局面 | `/inbox` / `/morning` / `/view` |
| 4 | 长任务不可恢复 | 进程退出或人离开后不知道任务状态 | TaskSnapshot + AuditLog + SQLite state |
| 5 | 多项目上下文串线 | 一个 agent 在 A 项目的经验污染 B 项目 | Project + Appointment-scoped session |
| 6 | 角色职责不稳定 | PM、reviewer、tester 语义靠 prompt 临时维持 | RoleProfile + ProjectRoleOverride |
| 7 | 经验不能复用 | 每个 role 从零开始踩坑 | MemoryKind fact/experience + ExperienceLayer |
| 8 | 协作不可追踪 | 子任务和父任务关系丢失 | Collaboration child task + audit event |

前两项决定我敢不敢任命 lead:如果 lead 只是把问题原样转给我,它就没有降低认知负担;如果风险动作不可控,我也不敢让它在离线场景里推进。
