...
```text
/task <parent_short_task_id>
/task <child_short_task_id>
```

预期:
- parent task 详情的 `Collaboration` 小节展示 reviewer child task 和 `/task <child_short_task_id>` 入口。
- child task 详情的 `Collaboration` 小节展示 `requested by: implementer` 和 parent `/task <parent_short_task_id>` 入口。
...
6. 可选:绑定已有 Codex provider session。
...
- 如果停在 `Task accepted: <task_id> [reviewer]`,先查 `/status` 或 `/inbox`。若 reviewer/Codex 长时间 running 且无输出,IM 应周期性出现 `Still running: no adapter output...`;这说明任务真实仍在跑但 provider 暂无 stdout。需要叫停时发送 `/interrupt <short_task_id>`,再检查 `logs/aico.log` 中该 task 是否停在 `Stream start` 且没有 stdout chunk。Round 114 起,Codex 默认 1800 秒无 stdout 才会返回 `adapter output idle timeout after 1800s` 并释放并发槽位;可用 `AICO_CODEX_OUTPUT_IDLE_TIMEOUT_SECONDS` 调整,设为 `0` 可禁用自动 idle timeout。若连续 heartbeat 但最小 `codex exec` 可正常返回,优先确认已重启到 Round 116 之后的代码,因为 CLI 子进程必须显式关闭 stdin,否则 Codex 可能等待 inherited stdin 的额外输入。
