...
- **离线托管**:用 `/overnight` 睡前下任务,之后通过 `/daily`、`/tasks`、`/audit`
  查看交付情况。
...
- **像 release room 一样维护开源项目**:任命 PM、implementer、tester、reviewer、
  release manager,再用 `/ask`、`/tasks`、`/daily`、`/audit` 推进一个小版本发布。
- **睡前托管一个 bugfix**:用 `/overnight` 把范围清楚的 bugfix 交给当前项目 lead,
  写文件和 shell 仍走 `/approve`,第二天看 `/daily` 验收。
...
- 离线托管:`/overnight` 工单已存在,持久化和重启恢复仍在进行中。
...
- 复剪更干净的公开 Release Room GIF。
- 让 `/overnight` 工单支持重启恢复。
- 完成飞书生产 callback smoke test。
- 补齐更适合外部贡献者的 setup docs、PR template 和 good first issue 列表。
- 补 adapter authoring 文档和无 token 本地 demo 路径。
